...
plt.plot(data, label='origin', color='black', linewidth=2)
plt.plot(data1, label='start_pivot', color='red', linewidth=1.5)
plt.plot(data2, label='end_pivot', color='blue', linewidth=1.5)
plt.legend(prop={'size':16}, ncol=1, loc=3)
plt.xlim([-1,len(data)])
plt.xticks(range(len(ticks))[1::5], ticks[1::5], fontsize=24)
plt.yticks(arange(5.0,5.5,0.1),fontsize=26)
plt.ylabel('PET [mm/d]', fontsize=26)
